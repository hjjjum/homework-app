...
- **js/rewards.js** — 성취 연출(진행 링·스티커 판·완주 축하·연속 달성).
  꾸미기(테마·배경·글꼴·스티커 판)는 **그 기기의 localStorage에만** 둔다(`hw.rewards.<studentId>`).
  **연속 기록만 Firestore에도 둔다**(`students/{id}/meta/streak`) — 아이의 기록이라 앱을 지웠다
