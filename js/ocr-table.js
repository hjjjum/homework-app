 ...
 */
const OCR_FIXES = [
  [/[들듣]린(?=\s*(?:문제|부분|것|답))/g, "틀린"],     // 틀린 문제 → 들린/듣린 문제
  [/오담(?=\s*노트)/g, "오답"],                        // 오답노트 → 오담노트
  [/(\d\s*[~\-]\s*\d+)\s*%/g, "$1쪽"],              // 124~139쪽 → 124~139%
  [/\b0\.(?=\d{1,3}\s*[~\-]\s*\d)/g, "p."],           // p.22~35 → 0.22~35 (쪽 번호 앞의 p)
];
...
export function cleanLine(text) {
  ...
  // 닫는 괄호가 } 나 ] 로 읽히는 일이 잦다. 열어 둔 괄호가 남아 있으면 되돌린다.
  // ("...(틀린 문제 옆에 답의 근거 쓰기}" → "...)"). 교재명 "[...]"은 여는 ( 가 없어 그대로 둔다.
  if (/[}\]]\s*$/.test(out) && out.split("(").length > out.split(")").length) {
    out = out.replace(/[}\]]\s*$/, ")");
  }
  return out;
}
